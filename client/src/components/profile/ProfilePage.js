...
export default function ProfilePage(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userPromise = fn.fetchUserProfile(userId);
        const ratingPromise = fn.getRating(userId);

        const [user, rating] = await Promise.all([userPromise, ratingPromise]);
      ...
      <div className="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
        <div className="max-w-md mx-auto">
          ...
          <div className="mt-5">
            ...
            {isAnEmployer() && !isProfileOwner() && subscribed && (
              <button className="mt-4 ml-4 bg-green-500 text-white px-6 py-2 rounded-md">
                Subscribed!
              </button>
            )}
          </div>
